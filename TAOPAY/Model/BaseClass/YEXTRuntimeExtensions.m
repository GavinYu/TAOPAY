...
#import "YEXTRuntimeExtensions.h"

#import <Foundation/Foundation.h>

y_propertyAttributes *y_copyPropertyAttributes (objc_property_t property) {
  ...
  y_propertyAttributes *attributes = calloc(1, sizeof(y_propertyAttributes) + typeLength + 1);
  if (!attributes) {
    fprintf(stderr, "ERROR: Could not allocate y_propertyAttributes structure for attribute string \"%s\" for property %s\n", attrString, property_getName(property));
    return NULL;
  }
  ...
  while (next && *next == ',') {
    ...
    switch (flag) {
        ...
        case 'C':
        attributes->memoryManagementPolicy = y_propertyMemoryManagementPolicyCopy;
        break;
        
        case '&':
        attributes->memoryManagementPolicy = y_propertyMemoryManagementPolicyRetain;
        break;
